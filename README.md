...
+ Currently we have to translate every update frame as there is no way to know,
  wether or not a translation might not be different the next week (due to a callback or so).
  ...
  But what about all the other situations. Some examples and ideas on how we can further optimize this:
    ...
    + Global variables are currently also not optimized away. But why not? Many of them will be static. So there should be a way
      to also cache the computed results of objects using global values.
    + For now we only optimize a string value when all expressions in that string value can be optimized away,
      but why not allow a partial evaluation? This way we can optimize away whatever we can optimize away.
